...
const DisplayTodos = ({
  ...
  const handleTasksStatus = () => {
    ...
    if (parsePercentage === 0) {
      ...
      return "Half of the tasks completed";
    } else if (parsePercentage > 50) {
  ...
  return (
    <>
      {data.length ? (
        <div>
          <div className="max-md:container border text-white max-w-[700px] mt-10 max-sm:mt-2 m-auto rounded-3xl bg-gradient-to-r from-blue-800 to-violet-700 p-10 max-sm:p-5">
            ...
            <div className="flex flex-col w-[60%] max-sm:w-[100%] mt-7 max-sm:mt-5">
              <div className="flex justify-between items-center">
                <p className=" max-sm:text-xs">
                  Progress
                  <span className={` text-m max-m:text-xs ${handleTasksStatus() === "No tasks completed" ? "text-green-400" : handleTasksStatus() === "Less than half tasks completed" ? "text-red-700" : "text-green-500"} font-semibold`}> ({handleTasksStatus()})</span>
                </p>
